import React from "react";
import Head from "next/head";
import Customer from "../../src/components/pages/Customer";
import { requestCategories } from "../../src/services";

const Pages = ({ content }) => (
  <React.Fragment>
    <Head>
      <title>Geração Pet - Minha Conta</title>
      <meta property="og:title" content="Geração Pet - Minha Conta" />
      <link rel="icon" href="/favicon.ico" />
      <meta
        name="description"
        content="Geração Pet - Minha Conta"
      />
      <meta
        property="og:description"
        content="Geração Pet - Minha Conta"
      />
    </Head>
    <Customer content={content} page="index"/>
  </React.Fragment>
);

export const getStaticProps = async () => {
  const response = await requestCategories();

  return {
    props: { content: { data: { categories: response.data } } },
  };
};

export default Pages;